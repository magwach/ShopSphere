...
const GiftCouponCard = () => {
  ...
  const filteredCoupon = coupon.filter(
  ...
  useEffect(() => {
    if (coupon.length) setUserInputCode(coupon?.[0].code);
  }, []);
      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            value={userInputCode}
            onChange={(e) =>
              setUserInputCode(e.target.value) && removeCoupon(type)
            }
            required
          />
        </div>
